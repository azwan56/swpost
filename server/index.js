...
app.post('/api/ai/tts', async (req, res) => {
  try {
    ...
    console.log(`[TTS] Requesting speech synthesis for text: "${text.substring(0, 15)}..." using voice ${voice}`);

    // Call Alibaba Cloud Model Studio CosyVoice TTS API
    ...
    if (!response.ok) {
      const errText = await response.text();
      console.error(`[TTS] DashScope API returned error (${response.status}):`, errText);
    ...
    res.status(500).json({ error: `语音配音生成失败: ${error.message}` });
  }
});
